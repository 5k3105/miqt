...
2b. **cflags: many KF6 libs ship NO pkg-config `.pc`** (KDE is CMake-based — e.g.
   KSyntaxHighlighting, KWidgetsAddons have none; KF6CoreAddons does). So
   `pkgConfigCflags("KF6<Lib>")` returns EMPTY → clang gets no includes → fails (or, with a
   bad `srcDir`, an empty package). Instead feed **Qt6 cflags + manual `-I`**:
   `"--std=c++17 -I/usr/include/KF6 -I/usr/include/KF6/<Lib> "+pkgConfigCflags("Qt6Widgets")`.
   Some libs also need the **parent** dir for a generated `*_version.h` (KSyntaxHighlighting's
   `_export.h` pulls `<ksyntaxhighlighting_version.h>` from `/usr/include/KF6/KSyntaxHighlighting`).
   **Validate before regen:** `clang++ -fsyntax-only --std=c++17 <your -I flags>
   $(pkg-config --cflags Qt6Widgets) <a header>.h` should exit 0.
...
5. **Keep allowlists tight** (WN-056). Bind only the classes you use; drop network/util/`*_export.h`.
   - **Header-dir casing varies per lib** — verify the real `srcDir` with
     `find /usr/include/KF6/<Lib> -name '*.h'` before trusting a path. Most KF6 libs use a
     lowercase subdir (`KTextEditor/ktexteditor/`, `KParts/kparts/`), but **KSyntaxHighlighting
     is CapitalCase** (`KSyntaxHighlighting/KSyntaxHighlighting/`) and KWidgetsAddons has the
     `.h` directly in `KWidgetsAddons/`. A wrong `srcDir` silently emits an EMPTY package (no
     panic). Keep build-time checks to `pkg-config --exists`, not hardcoded header paths.
